...
key trust;            // UUID of the current keyholder or NULL_KEY
...
Menu(key id) {
    ...
    string hdr = "信赖者：\n";
    integer length = llGetListLength(owned)-1;
    for (;length > -1;length--){
        hdr += "secondlife:///app/agent/"+llList2String(owned, length)+"/inspect \n";
    }
...
textbox(key id) {
    ...
    llTextBox(id,"输入UUID：",nDialogChannel);
    nDialogTimeout = DIALOG_TIMEOUT; // decreases every 10 seconds
    DEBUGF (0, "TBox", [id]);
}
...
Init() {
    ...
    nLock = 0;
    nDialogTimeout = 0;
    nhandle=llListen(-34343,"","","");
    llOwnerSay (llGetScriptName () + " ready");
    DEBUGF (0, "Init", []);
}

CheckAuth(key id){
    integer inowner = 1+llListFindList(owned, [id]);
    if (inowner) Release(id);
}
...
Release(key id){
        ...
        string hdr = "取走"+"secondlife:///app/agent/"+(string)llGetOwner()+"/inspect"+"的钥匙么？\n";
...
Addowner(key id){
    if (addpeople == 1){
        integer i = llListFindList(owned, [id]);
        if (i == -1){
            owned+=[id];
        }
    }
        ...
        integer i = llListFindList(owned, [id]);
        if (i != -1){
            owned = llDeleteSubList(owned, i, i);
        }
    }
    addpeople = 0;
    llRegionSayTo(llGetOwner(), -34343, llDumpList2String(owned, "#"));
    Menu(llGetOwner());
}
    ...
    listen(integer channel, string name, key id, string msg) {
        ...
        if (channel == nDialogChannel) {
            ...
            if (msg == "Main...") {
               ...
               llMessageLinked(1,0, "Cmd:Take keys", id);
               llMessageLinked (LINK_SET, 0, "Toucher", id);
            }
            else {
                key input = (key)msg;
                if (input) Addowner(input);
            }
        }
        else if (channel == -34343){
...
//~         DEBUGF (1, "link_message", [sender_num, num, str, id]);
        if (str=="Lockable") {
            ...
            else if (num == -20) { // touched
                DEBUGN (3, llKey2Name (id) + " touched.");
                CheckAuth(id);
            }
