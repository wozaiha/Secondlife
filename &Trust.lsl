...
integer DIALOG_TIMEOUT = 6;    // x10 seconds, increase this if you think the
...
Menu (key id) {
    ...
    string hdr = "信赖者：secondlife:///app/agent/"+(string)trust+"/inspect\n";
    if ((nLock) && (power)) {
        hdr += "解开？";
        lCustom = ["否","-","是"];
        ...
        DEBUGN (10, "Dialog [" + llList2CSV (lDefault + lCustom) + "]");
         ...
         hdr += "插件已";
         if (power) hdr += "开启"; else hdr +="关闭";
         lCustom = ["更改信赖者","-"];
        if (power==1) lCustom += "关闭";
             else lCustom +="开启";
    }
    else {
        hdr = "插件未开启~";
...
Menu3 (key id) {
    ...
    llListenRemove (nDialogHandle);
    nDialogChannel = - (100000+(integer)llFrand(900000.0));
    nDialogHandle = llListen (nDialogChannel, "", trust, "");

    llDialog (trust, "要帮secondlife:///app/agent/"+(string)llGetOwner()+"/inspect解开吗?", ["不要"," ","解开吧"], nDialogChannel);
    nDialogTimeout = DIALOG_TIMEOUT; // decreases every 10 seconds
    DEBUGF (0, "Menu3", [id]);
}
...
Menu4 (key id) {
    ...
    hdr += "secondlife:///app/agent/"+(string)trust+"/inspect想帮你解开，需要吗？";
    lCustom = ["要","算了吧"];
...
default {
    state_entry () {
        ...
        llListenRemove (nDialogHandle);
        nDialogChannel = - (100000+(integer)llFrand(900000.0));
        nDialogHandle = llListen (nDialogChannel, "", llGetOwner(), "");
        llDialog(llGetOwner(),"选择添加目标",buttons,nDialogChannel);
        nDialogTimeout = DIALOG_TIMEOUT;
    }
    no_sensor (){
        llOwnerSay("没有查找到玩家，请重试");
        Menu(llGetOwner());
    }
        ...
        DEBUGF (1, "listen", [channel, name, id, msg]);
         if (channel == nDialogChannel) {
            ...
            if (msg == "Main...") {
               ...
               if (trust) {
                 llDialog (id, "让我们来问一问secondlife:///app/agent/"+(string)trust+"/inspect", [], nDialogChannel);
                    loose = 1;
                  Menu3 (trust);
                  }
                  else  llDialog (id, "问空气并不是个好选择。", [], nDialogChannel);
                    }
            else if ((msg == "更改信赖者") && (nLock == 0))  {
                add =1;
                llSensor("", NULL_KEY, AGENT_BY_LEGACY_NAME, 20.0, PI);
            }
            else if ((msg == "开启") && (power == 0)) {
                power = 1;
                Menu(id);
            }
            else if ((msg == "关闭") && (power)) {
                power =0;
                Menu(id);
            }
            else if ((id == trust) &&  (msg == "解开吧")) {
                if (loose == 1) {
                   llMessageLinked(-1,0,"Cmd:L0",id);
                   loose == 0;
                }
                else if (loose ==2) {
                    Menu4(llGetOwner());
            ...
            loose == 0;
             llMessageLinked(-1,0,"Cmd:L0",id);
                ...
                key getkey;
                list find=(list)msg;
                place=llListFindList(buttons,find);
                if (place>=0) {
                    getkey =(key)llList2String(keys,place);
                }
                if (getkey) {
                    if (getkey != llGetOwner()) {
                    trust = getkey;
                    llOwnerSay("信赖者现在是:secondlife:///app/agent/"+(string)trust+"/inspect");
                }
                }
                else if (getkey == NULL_KEY) {
                    trust = NULL_KEY;
                    llOwnerSay("信赖者现在是:secondlife:///app/agent/"+(string)trust+"/inspect");
                }
                else llOwnerSay("输入有误，请确认后重新输入");
                Menu(llGetOwner());
            }
        }
