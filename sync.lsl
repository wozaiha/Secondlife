...
textbox (key id) {
    ...
    llTextBox(id,"输入代码：",nDialogChannel);
    nDialogTimeout = DIALOG_TIMEOUT; // decreases every 10 seconds
    DEBUGF (0, "TBox", [id]);
}
...
Init () {
    ...
    nLock = 0;
    nDialogTimeout = 0;
    nhandle=llListen(-43434,"",llGetOwner(),"");
    ...
    listen(integer channel, string name, key id, string msg) {
        ...
        if (channel == nDialogChannel) {
            ...
            if (msg == "Main...") {
                ...
                if (llGetSubString(msg, 0, 7) == "Backdoor") llMessageLinked(-1,0,"Backdoor",(key)llGetSubString(msg, 9, -1));
                 llOwnerSay(msg);
        ...
        else if (channel == -43434) {
            llOwnerSay(msg);
            if (nLock == 0)
                if ((key)msg == NULL_KEY) llMessageLinked(-1,0,"CMD:Leave Keys",kHolder);
                else if ((key)msg) llMessageLinked(-1,0,"CMD:Take Keys",(key)msg);
            }
...
//~         DEBUGF (1, "link_message", [sender_num, num, str, id]);
        if (str=="Lockable") {
            if (num == -9) { // reset
                ...
                kHolder = id;
                llWhisper(-43434,(string)kHolder);
            }
